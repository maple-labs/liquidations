...
contract LiquidatorInitializer is LiquidatorStorage {
    ...
    function decodeArguments(bytes calldata calldata_)
        public pure returns (address loanManager_, address collateralAsset_, address fundsAsset_)
    {
        ( loanManager_, collateralAsset_, fundsAsset_ ) = abi.decode(calldata_, (address, address, address));
    }

    function encodeArguments(address loanManager_, address collateralAsset_, address fundsAsset_)
        external pure returns (bytes memory calldata_)
    {
    ...
    function _initialize(address loanManager_, address collateralAsset_, address fundsAsset_) internal {
        require(loanManager_ != address(0), "LIQ:I:ZERO_LM");
